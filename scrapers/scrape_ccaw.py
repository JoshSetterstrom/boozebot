...
def scrape_ccaw():
    print("Scraping ccaw...")
    request = requests.get('https://craftcellars.ca/product-category/whisky/american-whiskey/page/1/?orderby=date')
    ...
    for i in range(1, page_total+1):
        request = requests.get(f'https://craftcellars.ca/product-category/whisky/american-whiskey/page/{i}/?orderby=date')
        soup = BeautifulSoup(request.text, 'html.parser')
    ...
    with open("./data/ccaw.json", "r+") as file: ccaw = json.load(file)
    updates = compare_json(ccaw, new_list)
    with open("./data/ccaw.json", "w+") as file: json.dump(new_list, file, indent=2)
